...
// ── Generations ─────────────────────────────────────────────
function insertGeneration({ driverName, route, landmarks, highlights, tripDate, vehicleType, tone, prompt, aiResponse, title }) {
  run(
    `INSERT INTO generations
       (driver_name, route, landmarks, highlights, trip_date, vehicle_type, tone, prompt, ai_response, title)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [driverName, route, landmarks ?? null, highlights ?? null, tripDate ?? null,
     vehicleType ?? 'Sedan', tone ?? 'Adventurous', prompt, aiResponse, title ?? null]
  );
  return queryOne('SELECT last_insert_rowid() as id')?.id ?? null;
}

function getGenerations({ page = 1, limit = 12, search = '' } = {}) {
  const offset = (page - 1) * limit;
  const params = [];
  let where = '';

  if (search) {
    where = 'WHERE driver_name LIKE ? OR route LIKE ? OR title LIKE ?';
    const t = `%${search}%`;
    params.push(t, t, t);
  }
  ...
  const data  = query(
    `SELECT id, driver_name, route, landmarks, highlights, trip_date,
            vehicle_type, tone, title, rating, comment, created_at
